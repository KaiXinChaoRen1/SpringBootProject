...
public class PaginationQuery {
    ...
    // }
    //
    // private List<Person> getPageSelectQuery(Specification<Person> spec, Pageable
    // pageable) {
    ...
    private List<Person> pageSelectQuery(Pageable pageable, String name) {
        Page<Person> all = pr.findAll((root, cq, cb) -> cq.where(cb.equal(root.get("name"), name)).getRestriction(),
                pageable);
        return all.getContent();
    }
